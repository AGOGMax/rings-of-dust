# Rings of Dust — audiobook cast sheet

For the ElevenLabs Audiobooks **Multi cast** → **Character Casting** step.
Detection is AI and misses minor speakers; check every name below is in the
detected list before generating. Dialogue counts are rough ("NAME said/asked").

Voice notes are from `TTS_Ready/LORE_BIBLE.txt`. Cole's callsign is
...
neutral, no theatrical warmth. Prose is short declaratives — pick a voice that
lands a period without leaning on it.

## Principal cast (10+ lines)

| Character | Lines | Voice from the Bible | Casting note |
|---|---|---|---|
| **Cole Nova** ("Jackpot") | 89 | Stoic, dry, self-deprecating. Wraps grief in jokes. | 40s, low, unhurried. Never raises it. |
...
| **Petra** | 18 | No contempt, only accuracy. | 60s, even, unimpressed. |
| **Squirrel** | 15 | Sharper of the pair (Red is jokier). | Male, quiet, economical. |
...
## Supporting (under 10 lines) — still cast them

| Character | Voice |
|---|---|
| **Tyler Durden** (Colony) | Male, 50s, slow, sincere. Grief that has burned through everything flammable. |
| **Taylor Glenn** | Male, 50s, careful deadpan. Precise. |
| **Red** | Female, brisk, jokier than Squirrel. |
| **iRIS** | Clinical, cold, half-second latency. This is the ONLY voice that should sound synthetic. Becomes Gracie in Ch21 — separate character entry, separate voice. |
| **Vince** | Male, 40s, kind, steady. |
| **Dr. Bruce Hexo** | Male, 50s, tired, exact. |
| **Chancellor DeMint** | Military authority. Grips hands like a vise. Male, 60s. |
| **Greenbaum** | Male, 30s, wound-up. |
| **Klaus Verguld** | Minimal. One sentence that ends conversations. Male, 70s, faint German under American. |
| **Wilhelm Verguld** (First Spark) | Male, German, 1930s formality. |
| **Stephen Conrad** | Male, 40s, scientist, frightened. |
| **Bitboy** | Six limbs, mechanical. Lines are rare — see how detection handles him; a light robotic treatment is acceptable here, unlike iRIS. |
| **Anushka**, **Dayo**, **Sorokina**, **Croft**, **Fekete**, **Baroness Kessler**, Hendricks, Cody, the Eureka man | Bit parts. Let casting propose; just make sure none of them reuse a principal's voice. |
